...
function setpaths()
{
    ...
    case $ARCH in
        arm)
            ...
            toolchaindir=arm/arm-cortex_a15-linux-gnueabihf-linaro_4.9.3-2015.03/bin
            if [ -d "$gccprebuiltdir/$toolchaindir" ]; then
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var VALIDUS_VERSION)
        ZIPFILE=validus-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.validus.device=$VALIDUS_BUILD");
    then
...
function cmka() {
    if [ ! -z "$1" ]; then
        for i in "$@"; do
            case $i in
                validus|otapackage|systemimage)
                    mka installclean
                    mka $i
                    ;;
                *)
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/validus/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
